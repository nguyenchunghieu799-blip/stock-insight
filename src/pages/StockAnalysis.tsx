...
function KlineChart({ data, indicator }: { data: KlineData; indicator: IndicatorData | null }) {
  const dates = data.dates;
  const ohlc = dates.map((d, i) => [data.opens[i], data.closes[i], data.lows[i], data.highs[i]]);
  ...
  const option = {
    ...
    xAxis: [
      {
        ...
        axisLabel: { color: "#5a6e8a", fontSize: 10, formatter: (v: string) => v.slice(5) },
      },
      {
    ...
    series: [
      ...
      {
        type: "line", data: data.ma5, xAxisIndex: 0, yAxisIndex: 0,
        symbol: "none", lineStyle: { color: "#f59e0b", width: 1 },
      },
      {
        type: "line", data: data.ma10, xAxisIndex: 0, yAxisIndex: 0,
        symbol: "none", lineStyle: { color: "#06b6d4", width: 1 },
      },
      {
        type: "line", data: data.ma20, xAxisIndex: 0, yAxisIndex: 0,
        symbol: "none", lineStyle: { color: "#a855f7", width: 1 },
      },
      {
        type: "bar", data: data.volumes.map((v, i) => [i, v, data.closes[i] >= data.opens[i] ? 1 : -1]),
        xAxisIndex: 1, yAxisIndex: 1,
        itemStyle: { color: (params: any) => params.data[2] > 0 ? "#ef4444" : "#22c55e" },
      },
    ],
  };
