...
def ultimate_analysis(code: str):
    ...
    try:
        ...
        for ci, cn in [("000001","上证指数"),("399001","深证成指"),("399006","创业板指"),("000688","科创50")]:
            info = market.get(ci, {})
            if info:
    ...
    try:
        ...
        print(f"\n  📌 {code} 所属: {sector_full}")
        if sector_rank:
            print(f"     排名: #{sector_rank}/{total} ({label})  涨跌:{sector_score:+.2f}%  资金:{sector_flow:+.1f}亿")
        else:
            print(f"     排名: 东方财富数据不可用 ({label})")
        if rpct < 35:
            print(f"     🔴 板块弱势！个股逆势上涨难度大，建议降仓或观望")
    ...
    from .short_term import calc_turnover_signal, calc_consecutive_days, calc_tail_tendency
    ...
    print(f"  短线评分: {st.get('短线评分',0)} → {st.get('评级','')} | ATR占比: {st.get('ATR占比%',0):.1f}%")
    print(f"  组合信号: {combo.get('信号','')} (强度{combo.get('强度',0)}) | {combo.get('详情','')}")
    ...
    print(f"  主力: {'近5日'+f'{total_flow:+.2f}亿' if flow_ok else '无数据'} | 今日: {'流入' if total_flow>0 else '流出' if total_flow<0 else '无数据'}")

    # ── L1 技术面 ──
    from .analysis import calc_support_resistance, calc_stop_levels
    ...
    n60 = round(float((kline['收盘'].iloc[-1]/kline['收盘'].iloc[-61]-1)*100),2) if len(kline)>60 else 0
    print(f"\n  ═══ L1 技术面 ═══")
    print(f"  现价: {price:.2f} | 近5日: {n5:+.1f}% | 近20日: {n20:+.1f}% | 近60日: {n60:+.1f}%")
    print(f"  MACD: {tech.get('macd_signal','')} | RSI: {tech.get('rsi_value',50):.0f} | KDJ: {tech.get('kdj_signal','')}")
    print(f"  支撑: {[round(float(x),2) for x in sl[:2]]}  压力: {[round(float(x),2) for x in rl[:2]]}")
    stop = calc_stop_levels(price, atr, float(sl[0]), float(rl[0]))
    print(f"  止损: {stop.get('止损参考价',price*0.93):.2f} | 止盈: {stop.get('止盈参考价',price*1.07):.2f} | ATR: {atr:.2f}")

    # ── L2 量化评分 ──
    from .quant import composite_quant_score, calc_risk_metrics, evaluate_trading_style
    ...
    def gf(k): v=fs.get(k,{}); return round(float(v.get('score',0)),1) if isinstance(v,dict) else 0
    print(f"\n  ═══ L2 量化评分 ═══")
    print(f"  综合: {qs} → {qr} | 短线: {trading.get('short_term_score',50)}分 | 长线: {trading.get('long_term_score',50)}分 | 风格: {trading.get('style','')}")
    print(f"  动量: {gf('momentum')}  技术: {gf('technical')}  基本面: {gf('fundamental')}  量能: {gf('volume')}  风险: {gf('risk')}")
    print(f"  夏普: {risk.get('sharpe_ratio',0):.2f} | 回撤: {risk.get('max_drawdown_pct',0):.1f}% | 波动率: {risk.get('annualized_volatility_pct',0):.1f}%")
    ...
    try:
        ...
        if isinstance(nt,dict): nt_holders = nt.get('holders',[])
    except: pass
    print(f"\n  ═══ L3 基本面 & 国家队 ═══")
    print(f"  ROE: {roe:.2f}% | 基本面评分: {gf('fundamental'):.0f}")
    if nt_holders:
        print(f"  国家队: {'🏛️ ' + ', '.join(nt_holders[:5])}{'...' if len(nt_holders)>5 else ''} ({len(nt_holders)}家)")
    else:
        print(f"  国家队: 无")
    ...
    except: ai = {"ensemble_direction":"?","ensemble_confidence":50}
    debate = generate_bull_bear_debate({
        "quant_score": qs,"technical": {"macd_signal": tech.get('macd_signal',''),
        "kdj_signal": tech.get('kdj_signal',''),"rsi": tech.get('rsi_value',50),
        "near5d": n5,"near20d": n20,"ma_status": tech.get('均线',''),
        "resistance": rl,"price": price,"pe": 0},
        "fund_flow": {"direction": "流入" if total_flow>0 else "流出"},
        "ai_prediction": {"direction": ai.get("ensemble_direction","看涨"),
        "confidence": ai.get("ensemble_confidence",50)}})
    print(f"\n  ═══ NL 多空辩论 ═══")
    print(f"  🐂 多头({debate['bull']['score']}分): {'; '.join(debate['bull']['points'][:3])}")
    print(f"  🐻 空头({debate['bear']['score']}分): {'; '.join(debate['bear']['points'][:3]) if debate['bear']['points'] else '无'}")
    ...
    try:
        ...
        if ml.get("agreement","?") == "高":
            ...
            print(f"  {emoji} 三模型一致{ml.get('ensemble_direction','?')} | 置信{ml.get('ensemble_confidence',0):.0f}% | 一致性:高 ({ml.get('votes','?')})")
        else:
            print(f"  ⚠️ 分歧 | 投票: {ml.get('ensemble_direction','?')} | 置信{ml.get('ensemble_confidence',0):.0f}% | 一致性:{ml.get('agreement','?')} ({ml.get('votes','?')})")
    ...
    except Exception: print("  ML预测暂不可用")
    ...
    except Exception: print("  宏观数据暂不可用")
    ...
    print(f"  AI预测: {ml_dir} 置信{ml_conf:.0f}% 一致性:{ml_agree} | 主力5日: {flow_str} {flow_sig}")
    print(f"  ROE: {roe:.2f}% | ATR: {atr:.2f}")
    print(f"")
    print(f"  预测方向: {pred_dir}({pred_conf:.0f}%) | 周期: {pred_days} | 目标涨幅: +{target_pct}% → {target_price}")
    print(f"")
    print(f"  ┌────────────┬──────────┬──────────┬──────────┐")
    print(f"  │ 买入区间     │ 止损       │ 止盈       │ 持有天数   │")
    print(f"  ├────────────┼──────────┼──────────┼──────────┤")
    ...
    if sector_rank == 0:
        risks.append(f"板块排名数据不可用，无法评估板块联动风险")
    ...
    print(f"\n{'='*W}")
    print(f"  数据来源: 新浪财经(实时行情) | Baostock(行业分类) | 东方财富(板块/资金)")
    print(f"  免责声明: 以上分析仅供学习研究，不构成投资建议")
    print(f"{'='*W}\n")
