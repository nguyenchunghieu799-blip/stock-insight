...
def backtest_strategy(df, strategy_func, initial_capital=10000.0, position_pct=0.95, **params):
    """策略回测：传入策略函数，返回回测结果"""
    if df.empty or len(df) < 10:
        return {"summary": {}, "trades": [], "equity_curve": [], "final_capital": initial_capital}

    # 策略函数应返回带 position(-1/0/1) 或 signal(-1/0/1) 列的 DataFrame
    result_df = strategy_func(df.copy(), **params)
    if "position" not in result_df.columns:
        if "signal" in result_df.columns:
            result_df["position"] = result_df["signal"]
        else:
            return {"summary": {}, "trades": [], "equity_curve": [], "final_capital": initial_capital}

    # 确保 position 是 int
    result_df["position"] = result_df["position"].fillna(0).astype(int)
    ...
    # 提取 numpy 数组避免 iterrows() 的 Series 构造开销
    prices = result_df["收盘"].values
    positions = result_df["position"].values
    ...
    for i in range(len(prices)):
        ...
        if pos == 1 and not in_position:
            # 买入
            entry_capital = cash * position_pct
            shares = entry_capital / price
            cash -= entry_capital
            in_position = True
            entry_price = price
            ...
            idx = indices[i]
            holding_days = (idx - entry_date).days if hasattr(idx, 'days') else 0
            trades.append({
    ...
    final_capital = cash + shares * result_df["收盘"].iloc[-1]
    ...
    ret_series = pd.Series(equity_curve).pct_change().dropna()
    ann_ret = calc_annualized_return(ret_series)
    ann_vol = calc_annualized_volatility(ret_series)
    sharpe = calc_sharpe_ratio(ret_series)
    dd = calc_max_drawdown(pd.Series(equity_curve))
...
def backtest_ma_crossover(df, fast_ma=5, slow_ma=20, **kwargs):
    """MA金叉死叉策略"""
    def strategy(df):
        df = df.copy()
        df["signal"] = 0
        col_f = f"MA{fast_ma}"
        col_s = f"MA{slow_ma}"
        if col_f in df and col_s in df:
            df.loc[df[col_f] > df[col_s], "signal"] = 1
            df.loc[df[col_f] <= df[col_s], "signal"] = -1
        return df
    return backtest_strategy(df, strategy, **kwargs)
